...
void Contact::applyPositionChange(Vector3 linearChange[2], Vector3 angularChange[2], real penetration) {
    ...
    for (unsigned i = 0; i < 2; i++) if (body[i]) {
        ...
        if (angularMove[i] == 0) {
            ...
            Matrix3 inverseInertiaTensor; body[i]->getInverseInertiaTensorWorld(&inverseInertiaTensor);
...
void ContactResolver::adjustPositions(Contact *c, unsigned numContacts, real duration) {
    ...
    // iteratively resolve interpretations in order of severity.
            ...
            for (unsigned b = 0; b < 2; b++) if (c[i].body[b]) {
                ...
                for (unsigned d = 0; d < 2; d++) {
                    if (c[i].body[b] == c[index].body[d]) {
                        ...
                        // and negative otherwise (because we're subtracting the resolution)..
